...
parser.add_argument('--percent',help='Percent of top scoring structures to average over.',nargs='*',default=[15,5,40,1],type=int)
...
for file in os.path.listdir(dir_path):
    if file.endswith('.dcd') and file.startswith('refinement_'):
        ...
        df = pd.DataFrame(list(zip([traj_idx]*num_frames,np.arange(num_frames),scores)))
